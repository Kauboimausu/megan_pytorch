...
90/10 en conjuntos de entrenamiento y validación. Output:
...
def _init_worker(noises_dir, snr_levels, base_seed):
    ...
    # diverge each worker's RNG so noise/SNR choices are not correlated
...
def main():
    ...
    train_paths = clean_paths[n_valid:]
    print('Total: {}  train: {}  valid: {}'.format(
        len(clean_paths), len(train_paths), len(valid_paths)))
    ...
    ok = errs = 0
    with Pool(opts.workers,
              initializer=_init_worker,
              initargs=(opts.noises_dir, opts.snr_levels, opts.seed)) as pool:
