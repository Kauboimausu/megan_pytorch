...
class Additive(object):
    ...
    def __init__(self, noises_dir, snr_levels=[0, 5, 10], do_IRS=False):
        ...
        if len(noises) == 0:
            ...
            for n_i, npath in enumerate(noises, start=1):
                ...
                self.noises.append({"file": npath, "data": nwav.astype(np.float32)})
    ...
    def __call__(self, wav, srate=16000, nbits=16):
        ...
        if wav.ndim > 1:
            wav = wav.reshape((-1,))
        noisy, noise_bound = self.addnoise_asl(
            wav, noise, srate, nbits, snr, do_IRS=self.do_IRS
        )
        # normalize to avoid clipping
        if np.max(noisy) >= 1 or np.min(noisy) < -1:
            small = 0.1
            while np.max(noisy) >= 1 or np.min(noisy) < -1:
                noisy = noisy / (1.0 + small)
                small = small + 0.1
    ...
    def addnoise_asl(self, clean, noise, srate, nbits, snr, do_IRS=False):
        ...
        if noise_len <= x_len:
            ...
            raise ValueError("Noise length has to be greater than speech " "length!")
    ...
    def apply_IRS(self, data, srate, nbits):
        ...
        overallGainFilter = interp1d(align_filter_dB[:, 0], align_filter[:, 1], 1000)
        ...
        factorDb = (
            interp1d(
                align_filter_dB[:, 0],
                align_filter_dB[:, 1],
                list(range(0, (pow_of_2 / 2) + 1) * freq_resolution),
            )
            - overallGainFilter
        )
    ...
    def asl_P56(self, x, srate, nbits):
        ...
        c = 2.0 ** (np.array(list(range(-15, (thres_no + 1) - 16))))
        ...
        for j in range(1, Delta.shape[0]):
            if a[j] != 0:
                ...
                if Delta[j] <= M:
                    ...
                    asl_ms_log, cl0 = self.bin_interp(
...
def PESQ(ref_wav, deg_wav):
    ...
    try:
        p = run(
            ["pesqmain".format(curr_dir), ref_tfl, deg_tfl, "+16000", "+wb"],
            stdout=PIPE,
            encoding="ascii",
        )
...
def SNR(ref_wav, deg_wav):
    ref = torch.as_tensor(ref_wav, dtype=torch.float32).reshape(-1)
    deg = torch.as_tensor(deg_wav, dtype=torch.float32).reshape(-1)
    return float(signal_noise_ratio(deg, ref))

def SI_SDR(ref_wav, deg_wav):
    ref = torch.as_tensor(ref_wav, dtype=torch.float32).reshape(-1)
    deg = torch.as_tensor(deg_wav, dtype=torch.float32).reshape(-1)
