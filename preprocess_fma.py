...
def main():
    ...
    parser.add_argument('--src', default='data/raw_audio/fma_small',
                        help='Directorio que contiene folders FMA')
    parser.add_argument('--dst', default='data/clean_fma',
                        help='Directio destino para los .wavs')
    ...
    print('Found {} mp3 files under {}'.format(len(mp3_paths), opts.src))
    if not mp3_paths:
        raise SystemExit('No se encontraron archivos mp3s.')

    tasks = [(p, opts.dst) for p in mp3_paths]
    ok = skipped = errored = 0
    with Pool(opts.workers) as pool:
        for i, (track_id, status) in enumerate(pool.imap_unordered(process_one, tasks), 1):
            if status == 'ok':
                ok += 1
            elif status == 'skip':
                skipped += 1
            ...
            if i % 200 == 0 or i == len(tasks):
                print('{}/{}  ok={} skip={} err={}'.format(
                    i, len(tasks), ok, skipped, errored))
